...
class _CryptoEncoder extends Converter<Map<String, dynamic>, String> {
  final String key;

  _CryptoEncoder(this.key);
...
class _CryptoDecoder extends Converter<String, Map<String, dynamic>> {
  final String key;

  _CryptoDecoder(this.key);
...
class _CryptoCodec extends Codec<Map<String, dynamic>, String> {
  _CryptoEncoder? _encoder;
  _CryptoDecoder? _decoder;

  _CryptoCodec(String? password);

  /// A non null [password] to use for the encryption/decryption
  _XXTeaCodec(String password) {
    _encoder = _CryptoEncoder(password);
    _decoder = _CryptoDecoder(password);
  }
...
/// ```
SembastCodec getCryptoCodec({@required String? password}) =>
    SembastCodec(signature: 'cryptography', codec: _CryptoCodec(password));
/* 
